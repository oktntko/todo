...
type Props = {
  modelValue: RouterOutput['whiteboard']['get'];
};
const props = satisfiesKeys<Props>()('modelValue');

const emits = {
  'update:modelValue': (_: RouterOutput['whiteboard']['get']) => true,
} satisfies EmitsType;

export default defineComponent(
  ($props: Props, { emit: $emit }) => {
    const currentWhiteboard = useVModel($props, 'modelValue', $emit);
    ...
    const refCanvasContainer = useTemplateRef<HTMLDivElement>('refCanvasContainer');
    const { height, width } = useElementSize(refCanvasContainer);

    watch([() => height.value, () => width.value], ([height, width]) => {
      ...
      <>
        ...
        <Teleport to="#whiteboard-edit">
          <div class="flex grow flex-col gap-0.5 px-2">
            <label for="background" class="text-xs capitalize">
              background
            </label>
            <input
              id="background"
              v-model={control.value.background}
              list="color-picker"
              type="color"
              class="block w-full rounded-lg border border-gray-300 bg-white p-1 text-gray-900 sm:text-sm"
              onInput={() =>
                setActiveObjectsProperty(
                  'fill',
                  R.rgba(control.value.background, control.value.backgroundAlpha),
                )
              }
            />
          </div>
          <div class="flex grow flex-col gap-0.5 px-2">
            ...
            <input
              id="background-alpha"
              v-model={control.value.backgroundAlpha}
              type="range"
              min="0"
              max="1"
              step="0.1"
          ...
          <div class="flex grow flex-col gap-0.5 px-2">
            <label for="stroke" class="text-xs capitalize">
              stroke
            </label>
            <input
              id="stroke"
              v-model={control.value.stroke}
              list="color-picker"
              type="color"
              class="block w-full rounded-lg border border-gray-300 bg-white p-1 text-gray-900 sm:text-sm"
              onInput={() => setActiveObjectsProperty('stroke', control.value.stroke)}
            />
          </div>
          <div class="flex grow flex-col gap-0.5 px-2">
            <label for="stroke-width" class="text-xs capitalize">
              stroke width
            </label>
            <input
              id="stroke-width"
              v-model={control.value.strokeWidth}
              type="range"
              min="0"
              max="10"
              class="block w-full rounded-lg border border-gray-300 bg-white p-1 text-gray-900 sm:text-sm"
              onInput={() => setActiveObjectsProperty('strokeWidth', control.value.strokeWidth)}
            />
          </div>

          <div class="col-span-2 flex grow flex-col gap-0.5 px-2">
            <label for="opacity" class="text-xs capitalize">
              opacity
            </label>
            <input
              id="opacity"
              v-model={control.value.opacity}
              type="range"
              min="0"
              max="1"
              step="0.1"
              class="block w-full rounded-lg border border-gray-300 bg-white p-1 text-gray-900 sm:text-sm"
              onInput={() => setActiveObjectsProperty('opacity', control.value.opacity)}
            />
          </div>
          ...
          <div class="col-span-2 flex grow flex-col gap-0.5 px-2">
            ...
            <div class="flex gap-1.5">
              <button
                ...
                onClick={() => setActiveObjectsLayer('sendObjectToBack')}
              >
              ...
              <button
                ...
                onClick={() => setActiveObjectsLayer('sendObjectBackwards')}
              >
              ...
              <button
                ...
                onClick={() => setActiveObjectsLayer('bringObjectForward')}
              >
              ...
              <button
                ...
                onClick={() => setActiveObjectsLayer('bringObjectToFront')}
              >
          ...
          <div class="shrink-0">
            <div class="flex grow flex-col gap-0.5 px-2">
              ...
              <div class="flex flex-row items-center gap-2">
                <input
                  id="background-alpha"
                  v-model={control.value.zoom}
                  type="range"
                  min="0.1"
                  max="2"
                  step="0.1"
                  class="block w-full rounded-lg border border-gray-300 bg-white p-1 text-gray-900 sm:text-sm"
                  onInput={handleZoomChanged}
                />
                <span class="w-8 shrink-0 text-right text-xs">
                  {Math.floor(control.value.zoom * 100)}%
                </span>
              </div>
            </div>
          </div>
        ...
        <div ref="refCanvasContainer" class={['relative h-full w-full border bg-white']}>
          ...
          <div class="absolute right-0 bottom-2 left-0 inline-flex justify-center gap-2">
            <button
              type="button"
              class={[
                'relative inline-flex h-9 w-9 cursor-pointer items-center justify-center rounded-lg border border-gray-400 p-1.5 transition-colors hover:bg-gray-200',

                {
                  'bg-blue-100 shadow ring-2 shadow-blue-400 ring-blue-400 hover:bg-blue-200!':
                    refstate.value.mode === 'pan' || refstate.value.mode === 'panning',
                },
              ]}
              title="pan"
              onClick={(e) => {
            ...
            <button
              ...
              class={[
                ...
                {
                  'bg-blue-100 shadow ring-2 shadow-blue-400 ring-blue-400 hover:bg-blue-200!':
                    refstate.value.mode === 'object:draw:rectangle',
                },
              ]}
            ...
            <button
              ...
              class={[
                ...
                {
                  'bg-blue-100 shadow ring-2 shadow-blue-400 ring-blue-400 hover:bg-blue-200!':
                    refstate.value.mode === 'object:draw:circle',
                },
              ]}
            ...
            <button
              ...
              class={[
                ...
                {
                  'bg-blue-100 shadow ring-2 shadow-blue-400 ring-blue-400 hover:bg-blue-200!':
                    refstate.value.mode === 'object:draw:triangle',
                },
              ]}
            ...
            <button
              ...
              class={[
                ...
                {
                  'bg-blue-100 shadow ring-2 shadow-blue-400 ring-blue-400 hover:bg-blue-200!':
                    refstate.value.mode === 'object:draw:line',
                },
              ]}
            ...
            <button
              ...
              class={[
                ...
                {
                  'bg-blue-100 shadow ring-2 shadow-blue-400 ring-blue-400 hover:bg-blue-200!':
                    refstate.value.mode === 'object:draw:text',
                },
              ]}
              onClick={(e) => {
                e.preventDefault();
