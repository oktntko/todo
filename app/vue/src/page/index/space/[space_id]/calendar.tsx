...
type Props = {
  todo_list: RouterOutput['todo']['list'];
};
const props = satisfiesKeys<Props>()('todo_list');
...
const Page = defineComponent(
  ($props: Props) => {
    ...
    async function onEventClicked(args: EventArgs) {
      ...
      const index = todo_list.findIndex((x) => x.todo_id === result.todo.todo_id);
      if (result.event === 'delete') {
        todo_list.splice(index, 1);
      } else {
        todo_list.splice(index, 1, result.todo);
      }
    ...
    async function onEventChanged(args: EventChangedArgs) {
      const current = todo_list.find((v) => v.todo_id === args.e.id().toString());
      ...
      try {
        ...
        const index = todo_list.findIndex((v) => v.todo_id === todo.todo_id);
        if (index !== -1) {
          todo_list.splice(index, 1, todo);
        }
...
export default defineComponent(() => {
  ...
  const todo_list = ref<RouterOutput['todo']['list']>();
  onMounted(async () => {
    const result = await trpc.todo.list.query({
      space_id: $route.params.space_id,
    });
    todo_list.value = result;
  });
  ...
  return () => (
    ...
    >
      {todo_list.value ? <Page todo_list={todo_list.value} /> : <MyLoading />}
    </Transition>
  );
});
