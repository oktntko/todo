    ...
    <div class="mb-8 flex flex-col gap-4 px-4">
      <div>
        <nav aria-label="Breadcrumb">
          <MyBreadcrumb class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            >
              <span class="icon-[vaadin--folder-open] h-3 w-3 transition"> </span>
        ...
        >
          <section class="flex flex-col gap-3">
            <div class="focus-container flex flex-col gap-0.5">
              <div>
                <label for="where.file_keyword" class="optional text-sm capitalize">
                  keyword
                </label>
              </div>
              ...
              <div>
                <MyInput
                  ...
                  v-model={modelValue.value.where.file_keyword}
                  class="w-full"
                  maxlength="100"
                />
              </div>
      ...
      <div class="">
        <header class="z-10 flex flex-row items-center gap-2 rounded-t border border-gray-300 bg-gray-50 p-2 px-4 py-2 text-sm">
          <MyButton type="button" color="blue" variant="outlined" onClick={handleUpload}>
          ...
          <MyButton
            ...
            onClick={async () => {
              const file_id_list = checkedList.value.map((x) => x.file_id);
          ...
          <MyButton
            ...
            onClick={async () => {
              ...
              loading.value = true;
              try {
                await trpc.file.deleteMany.mutate({
                  space_id: $route.params.space_id,
        ...
        <table class="h-full w-full overflow-x-auto rounded-sm border border-gray-300 bg-gray-50 text-sm">
                ...
                <label class="flex h-full justify-center hover:cursor-pointer">
                  <input
                    ...
                    onClick={() => {
                      ...
                      data.value.file_list.forEach((x) => (x.checked = !checked));
                    }}
                  />
                </label>
              </th>
              ...
              >
                <div class="flex items-center justify-center">
                  ...
                  <MySortButton
                    ...
                    field="filename"
                    onClick={() => changeSortOrder('filename')}
                  ></MySortButton>
                </div>
              </th>
              <th
              ...
              >
                <div class="flex items-center justify-center">
                  ...
                  <MySortButton
                    sort={modelValue.value.sort}
                    field="filesize"
                    onClick={() => changeSortOrder('filesize')}
                  ></MySortButton>
                </div>
              </th>
              <th
          ...
          <tbody class="divide-y divide-gray-200 bg-white text-gray-900">
            {data.value.file_list.map((file, i) => (
              <tr key={file.file_id} class="divide-x divide-gray-200">
                <td class="max-w-48">
                  <button
                ...
                <td class="max-w-48">
                  <div
                    ...
                    title={file.mimetype}
                  >
                    <span class={['h-5 w-5 shrink-0', MimetypeIcon(file)]}></span>
                    <span class="text-xs text-gray-500">{file.mimetype}</span>
                  </div>
                  <MimetypePreview file_id={file.file_id} mimetype={file.mimetype} />
                </td>
                <td class="max-w-48">
                ...
                <td class="max-w-48">
                  <div class="line-clamp-1 min-w-32 px-2 py-1">
                    {file.created_at
                      ? dayjs(file.created_at).format('YYYY-MM-DD hh:mm:ss.SSS')
                      : ''}
                  </div>
                </td>
                <td class="max-w-48">
                  <div class="line-clamp-1 min-w-32 px-2 py-1">
                    {file.updated_at
                      ? dayjs(file.updated_at).format('YYYY-MM-DD hh:mm:ss.SSS')
                      : ''}
                  </div>
                </td>
              </tr>
