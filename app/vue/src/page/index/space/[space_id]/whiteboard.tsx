...
type Props = {
  whiteboard_list: RouterOutput['whiteboard']['list'];
};
const props = satisfiesKeys<Props>()('whiteboard_list');
...
const Page = defineComponent(
  ($props: Props) => {
    ...
    const whiteboard_list = ref([...$props.whiteboard_list]);
...
export default defineComponent(() => {
  ...
  onMounted(async () => {
    const result = await trpc.whiteboard.list.query({
      space_id: $route.params.space_id,
    });
    whiteboard_list.value = result;
  });
  ...
  return () => (
    ...
    >
      {whiteboard_list.value ? <Page whiteboard_list={whiteboard_list.value} /> : <MyLoading />}
    </Transition>
  );
});
